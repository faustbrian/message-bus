...
final class BusServiceProvider extends ServiceProvider
{
     ...
     *
     * Laravel 10 compatibility: manually register singletons
     * (Laravel 11+ uses #[Singleton] attribute which is ignored in Laravel 10).
     */
    public function register(): void
    {
        $this->app->singleton(\Cline\MessageBus\Commands\CommandBus::class);
        $this->app->singleton(\Cline\MessageBus\Queries\QueryBus::class);
    }

    public function boot(): void
    {
        // Map handlers via Laravel Bus, preferring cached maps, falling back to local discovery in dev
        $commandMap = $this->loadCachedHandlers('command-handlers') ?? $this->discoverIfLocal('command-handlers');
        $queryMap = $this->loadCachedHandlers('query-handlers') ?? $this->discoverIfLocal('query-handlers');
     ...
     */
    private function loadCachedHandlers(string $type): ?array
    {
        $path = match ($type) {
